CREATE OR REPLACE VIEW osm_unique_road AS (
    SELECT rp.osm_id, rp.geometry, rp.type, rp.service,
           rp.oneway, rp.is_bridge, rp.is_tunnel, rp.is_ford, rp.timestamp,
    ...
    SELECT rl.osm_id, rl.geometry, rl.type, rl.service,
           rl.oneway, rl.is_bridge, rl.is_tunnel, rp.is_ford, rl.timestamp,
...
CREATE OR REPLACE VIEW road_z5toz6 AS
    SELECT osm_id, geometry, type, service, oneway, 'none' AS structure, z_order, timestamp
...
CREATE OR REPLACE VIEW road_z13 AS
    SELECT osm_id, geometry, type, service, oneway, classify_structure(is_tunnel, is_bridge, is_ford) AS structure, z_order, timestamp
    FROM osm_unique_road
